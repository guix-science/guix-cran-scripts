...
definition."
  (format #t "Importing package ~a from CRAN/Bioconductor…~%" cran-name)
...
(define (create-channel output-dir channel-name channel-url missing)
  ...
  (define new-sexps (map import-package missing))
  ...
  (write-channel-file output-dir channel-url))
...
(define* (main output-dir channel-name #:optional (type 'cran))
  ...
  (let* ((packages (case type
    ...
    (let ((channel-url
           (case type
             ((bioc) "https://github.com/guix-science/guix-bioc.git")
             ((cran) "https://github.com/guix-science/guix-cran.git"))))
      (create-channel output-dir channel-name channel-url missing-packages))
